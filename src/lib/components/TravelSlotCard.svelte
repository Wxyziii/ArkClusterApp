<script lang="ts">
  ...
  let {
    slot,
    map
  }: { slot: 'Home' | 'On-demand' | string; map: ArkMap | null } = $props();
...
<div class="card-elevated p-4 {isHome ? 'border-l-2 border-l-[#3a3a3a]' : ''}">
  ...
  {#if map}
    ...
    <div class="mt-2 space-y-1 text-xs text-[#8c8c8c]">
      <div class="flex justify-between"><span>Players</span><span class="tabular-nums text-[#ededed]">{map.playerCountSource === 'rcon' ? map.players : 'unknown'}/{map.maxPlayers ?? 'unknown'}</span></div>
  ...
  {:else}
    <p class="mt-3 text-sm text-[#5c5c5c]">No on-demand server running</p>
